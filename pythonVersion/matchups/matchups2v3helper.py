def analyze2v3(team1, team2, team1wins, db):
    for i in range(0, 5):
            ...
            for k in range(0, 5):
                for l in range(k + 1, 5):
                    for m in range(l + 1, 5):
                        ...
                        foe2_id = team2[l]
                        foe3_id = team2[m]
                        hero_entry = db.matchups2v3.find_one({'h1': str(hero_id), 'h2': str(hero2_id),
                                                              'h3': str(foe_id), 'h4': str(foe2_id), 'h5': str(foe3_id)})
                        if not hero_entry:
                        ...
                        db.matchups2v3.remove({'h1': str(hero_id), 'h2': str(hero2_id),
                                               'h3': str(foe_id), 'h4': str(foe2_id), 'h5': str(foe3_id)})
                        db.matchups2v3.insert_one(hero_entry)
