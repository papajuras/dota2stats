def analyze3v2(team1, team2, team1wins, db):
    for i in range(0, 5):
            ...
            for k in range(0, 5):
                for l in range(k + 1, 5):
                    for m in range(j + 1, 5):
                        ...
                        foe_id = team2[k]
                        foe2_id = team2[l]
                        hero_entry = db.matchups3v2.find_one({'h1': str(hero_id), 'h2': str(hero2_id),
                                                              'h3': str(hero3_id), 'h4': str(foe_id), 'h5': str(foe2_id)})
                        if not hero_entry:
                            hero_entry = {'s': {}, 'h1': str(hero_id), 'h2': str(hero2_id), 'h3': str(hero3_id),
                                          'h4': str(foe_id), 'h5': str(foe2_id)}
                        ...
                        db.matchups3v2.remove({'h1': str(hero_id), 'h2': str(hero2_id), 'h3': str(hero3_id), 'h4': str(foe_id), 'h5': str(foe2_id)})
                        db.matchups3v2.insert_one(hero_entry)
