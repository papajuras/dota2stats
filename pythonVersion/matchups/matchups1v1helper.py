...
def analyze1v1(team1, team2, team1wins, db):
    for hero_id in team1:
        for foe_id in team2:
            hero_entry = db.matchups1v1.find_one({'h1': str(hero_id), 'h2': str(foe_id)})
            if not hero_entry:
                hero_entry = {'s': {}, "h1": str(hero_id), "h2": str(foe_id)}
            ...
            db.matchups1v1.remove({'h1': str(hero_id), 'h2': str(foe_id)})
            db.matchups1v1.insert_one(hero_entry)
